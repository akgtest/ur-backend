...
router.get("/test", authenticateJWT, async (req, res) => {
  try {
    ...
    console.error("Test endpoint error", { error: error.message });
    res.status(500).json({
      success: false,
      error: "Test failed",
    });
...
router.put("/profile", authenticateJWT, async (req, res) => {
  try {
    ...
    console.info("Profile update request", {
      userId: req.user?.id,
      body: req.body,
    });
...
router.get("/watchlist", authenticateJWT, async (req, res) => {
  try {
    ...
    const processedWatchlist = (user.watchlist || [])
      .map((property) => {
        ...
        if (
          !propertyObj.subcategoryName &&
          propertyObj.category &&
          propertyObj.subcategory
        ) {
          try {
        ...
        if (propertyObj.type === "regular") {
          if (
            propertyObj.projectImages &&
            propertyObj.projectImages.length > 0
          ) {
          ...
          displayImage =
            propertyObj.wallpaperImage ||
            (propertyObj.projectImages && propertyObj.projectImages.length > 0
              ? propertyObj.projectImages[0]
              : null);
        }
        ...
        if (propertyObj.projectImages) {
          propertyObj.projectImages = propertyObj.projectImages.map((img) =>
            convertToCloudFrontUrl(img),
          );
        }
    ...
    console.error("Get watchlist error", { error: error.message });
    res.status(500).json({
      success: false,
...
router.post("/watchlist", authenticateJWT, async (req, res) => {
  try {
    ...
    const isAlreadyInWatchlist = user.watchlist.some(
      (id) => id.toString() === propertyIdStr,
    );

    if (!isAlreadyInWatchlist) {
      user.watchlist.push(propertyId);
      await user.save();
      console.info("Property added to watchlist", {
        userId: req.user.id,
        propertyId: propertyIdStr,
        propertyTitle: property.projectName || property.title,
      });
    } else {
      console.info("Property already in watchlist", {
        userId: req.user.id,
        propertyId: propertyIdStr,
      });
    }
    ...
    res.json({
      ...
      message: isAlreadyInWatchlist
        ? "Property already in watchlist"
        : "Property added to watchlist",
    });
  } catch (error) {
    console.error("Add to watchlist error", { error: error.message });
    res.status(500).json({
      success: false,
...
router.delete("/watchlist/:propertyId", authenticateJWT, async (req, res) => {
  try {
    ...
    console.info("Property removed from watchlist", {
      userId: req.user.id,
      propertyId,
    });
    ...
    console.error("Remove from watchlist error", { error: error.message });
    res.status(500).json({
      success: false,
...
router.get("/properties", authenticateJWT, async (req, res) => {
  try {
    ...
    console.error("Get user properties error", { error: error.message });
    res.status(500).json({
      success: false,
...
router.delete("/properties/:propertyId", authenticateJWT, async (req, res) => {
  try {
    ...
    const user = await User.findById(req.user.id);
    if (user) {
      user.myProperties = user.myProperties.filter(
        (id) => id.toString() !== propertyId,
      );
      await user.save();
    }
    ...
    console.error("Delete property error", { error: error.message });
    res.status(500).json({
      success: false,
