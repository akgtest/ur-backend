  ...
  try {
    ...
    if (!phoneRegex.test(cleanPhone)) {
      return res.status(400).json({
        success: false,
        error:
          "Invalid phone number format. Please enter a valid Indian mobile number (10 digits starting with 6-9)",
      });
    }
    ...
    if (result.success) {
      ...
      const dbFormattedPhone =
        result.dbFormattedPhone ||
        twoFactorService.formatPhoneNumber(phoneNumber);
      ...
      console.info(
        `OTP sent successfully to ${dbFormattedPhone} (Type: ${result.type})`,
      );
    ...
    console.error("Send OTP error", { error: error.message });
    res.status(500).json({
      success: false,
    ...
    if (result.success) {
      ...
      try {
        ...
        if (mongoose.connection.readyState !== 1) {
          console.error("Database not connected during OTP verification");
        ...
        if (!phoneRegex.test(session.phoneNumber)) {
          console.error(
            `Invalid phone number format for database: ${session.phoneNumber}`,
          );
        ...
        if (!user) {
          // Create new user
          console.info(`Creating new user with phone: ${session.phoneNumber}`);
          user = new User({
          ...
          await user.save();
          console.info(`New user created successfully: ${session.phoneNumber}`);
        } else {
          // Update last login
          console.info(
            `Updating last login for existing user: ${session.phoneNumber}`,
          );
          user.lastLogin = new Date();
          await user.save();
          console.info(
            `User login updated successfully: ${session.phoneNumber}`,
          );
        }
        ...
        res.cookie(
          "urbanesta_refresh_token",
          refreshToken,
          refreshCookieOptions,
        );

        console.info(`OTP verified successfully for ${session.phoneNumber}`);
        ...
        console.error("Database error during OTP verification:", {
        ...
        if (dbError.code === 11000) {
          console.error(`Duplicate phone number: ${session.phoneNumber}`);
          return res.status(409).json({
            success: false,
            error:
              "Phone number already exists. Please try logging in instead.",
          });
        }
...
export const verify2FactorSession = async (req, res, next) => {
  try {
    ...
    console.error("2Factor session verification error", {
      error: error.message,
    });
    res.status(401).json({
      success: false,
...
router.get("/me", verify2FactorSession, async (req, res) => {
  try {
    ...
    console.error("Get user info error", { error: error.message });
    res.status(500).json({
      success: false,
...
router.get("/balance", async (req, res) => {
  try {
    ...
    console.error("Get balance error", { error: error.message });
    res.status(500).json({
      success: false,
