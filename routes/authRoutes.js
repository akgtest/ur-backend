...
router.post("/send-otp", async (req, res) => {
  try {
    const { phone, name, city, propertyId, propertyName, propertyUrl } =
      req.body;
    ...
    console.info(`Sending OTP to ${phoneNumber} for user: ${name}`);
    ...
    if (result.success) {
      ...
      console.info(
        `OTP sent successfully to ${phoneNumber}, sessionId: ${sessionId}`,
      );
...
router.post("/verify-otp", async (req, res) => {
  ...
  try {
    ...
    console.info(`Verifying OTP for sessionId: ${sessionId}`);
    ...
    if (!result.success) {
      ...
      console.error(
        `OTP verification failed for sessionId: ${sessionId}, error: ${result.error || "Unknown error"}`,
      );

      // Provide user-friendly error messages
      let errorMessage =
        result.error ||
        `Invalid OTP. ${3 - session.attempts} attempts remaining.`;
    ...
    console.info(`OTP verified successfully for phone: ${session.phoneNumber}`);
    ...
    if (!phone10Digit || phone10Digit.length !== 10) {
      console.error(
        `Invalid phone number in session: ${JSON.stringify(session)}`,
      );
      return res.status(400).json({
        success: false,
    ...
    try {
      if (!user) {
        ...
        try {
          ...
          await user.save();
          console.info(
            `✅ New user created: ${user._id} - ${session.phoneNumber} with email: ${defaultEmail}`,
          );
        } catch (createError) {
          ...
          if (createError.code === 11000) {
            ...
            if (createError.message && createError.message.includes("email")) {
              ...
              console.info(
                `Email duplicate detected, trying with unique email: ${uniqueEmail}`,
              );
              ...
              try {
                ...
                await user.save();
                console.info(
                  `✅ New user created with unique email: ${user._id} - ${session.phoneNumber}`,
                );
              } catch (retryError) {
                // If still fails, find existing user by phone
                console.info(
                  `Retry failed, finding existing user: ${session.phoneNumber}`,
                );
                ...
                if (user) {
                  ...
                  await user.save();
                  console.info(
                    `✅ Existing user found and logged in: ${user._id} - ${session.phoneNumber}`,
                  );
                } else {
                  console.error(
                    `Duplicate error but user not found: ${session.phoneNumber}`,
                  );
                  throw createError;
                }
              }
              ...
              console.info(
                `User created in parallel, finding existing user: ${session.phoneNumber}`,
              );
              user = await User.findOne({ phoneNumber: session.phoneNumber });
              if (user) {
                isNewUser = false;
        ...
        if (updated) {
          await user.save();
          console.info(
            `✅ Returning user updated: ${user._id} - ${session.phoneNumber}`,
          );
        } else {
          console.info(
            `✅ Returning user logged in: ${user._id} - ${session.phoneNumber}`,
          );
        }
      }
    } catch (userError) {
      ...
      if (!user) {
        ...
        if (user) {
          ...
          user.lastLogin = new Date();
          await user.save();
          console.info(`✅ User found in last resort check: ${user._id}`);
        } else {
        ...
        console.warn(
          `User save failed but user exists, continuing: ${user._id}`,
        );
      }
    }
    ...
    try {
      ...
      await lead.save();
      console.info(
        `✅ Lead created: ${lead._id} for property: ${propertyName || "N/A"}`,
      );
    } catch (leadError) {
      ...
      console.warn(
        `⚠️ Lead creation failed but user is logged in: ${user._id}`,
      );
    ...
    try {
      ...
      console.info(`✅ JWT tokens generated for user: ${user._id}`);
    } catch (tokenError) {
      console.error("Error generating JWT tokens:", {
    ...
    console.info(`✅ Authentication cookies set for user: ${user._id}`);
    ...
    if (error.name === "ValidationError") {
      ...
      if (error.message && error.message.includes("email")) {
        errorMessage =
          "Account creation in progress. Please wait a moment and try again.";
      } else if (error.message && error.message.includes("phoneNumber")) {
        errorMessage =
          "An account with this phone number already exists. Please try logging in.";
      } else {
