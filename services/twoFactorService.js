...
class TwoFactorService {
  ...
  getApiKey() {
    ...
    if (!process.env.TWO_FACTOR_API_KEY) {
      ...
      throw new Error(
        "2Factor API key is required. Please check your environment configuration.",
      );
    }

    const apiKey = process.env.TWO_FACTOR_API_KEY.trim();
    if (!apiKey || apiKey.length < 10) {
      console.error("2Factor API key appears to be invalid or too short");
      throw new Error(
        "Invalid 2Factor API key. Please check your environment configuration.",
      );
    }
  ...
  async sendOTP(phoneNumber) {
    try {
      ...
      console.info(
        `Attempting to send SMS OTP to phone: ${apiFormattedPhone} (DB format: ${dbFormattedPhone})`,
      );
      ...
      if (smsResult.success) {
        console.info(
          `SMS OTP sent successfully to ${apiFormattedPhone}, sessionId: ${smsResult.sessionId}`,
        );
        return {
          success: true,
      ...
      console.error(
        `SMS OTP failed for ${apiFormattedPhone}: ${smsResult.error}`,
      );
      return {
        success: false,
  ...
  async sendSMSOTP(apiFormattedPhone) {
    try {
      ...
      console.info(
        `Sending SMS OTP via 2Factor API: ${apiUrl.replace(apiKey, "***")}`,
      );
      console.info(`Phone number format: ${apiFormattedPhone}`);

      const response = await axios.get(apiUrl, { timeout: 10000 });

      // Log full 2Factor response for debugging
      console.info(`2Factor SMS API Response:`, {
      ...
      if (response.data.Status === "Success") {
        ...
        if (!response.data.Details) {
          console.error("2Factor returned Success but no sessionId");
          return {
            success: false,
        ...
        console.info(`SMS OTP session created: ${response.data.Details}`);
        return {
          success: true,
        ...
        console.error(`2Factor SMS API Error - SMS delivery failed:`, {
          Status: response.data.Status,
          Details: response.data.Details,
          Message: response.data.Message,
        ...
        const errorMsg =
          ...
          "SMS delivery failed";
        console.error(
          `SMS OTP failed. Error: ${errorMsg}. Check template configuration in 2Factor dashboard.`,
        );
      ...
      console.error(`2Factor SMS API Exception:`, {
  ...
  async sendVoiceOTP(apiFormattedPhone) {
    try {
      ...
      return {
        success: false,
        error:
          error.response?.data?.Details || error.message || "Voice API error",
      };
    }
  }
  ...
  async verifyOTP(sessionId, otp) {
    try {
      ...
      console.info(`Verifying OTP for session: ${sessionId}`);

      const apiKey = this.getApiKey();
      const verifyUrl = `${this.baseUrl}/${apiKey}/SMS/VERIFY/${sessionId}/${otp}`;

      console.info(`2Factor Verify API: ${verifyUrl.replace(apiKey, "***")}`);
      ...
      console.info(`2Factor Verify API Response:`, {
        Status: response.data.Status,
        Details: response.data.Details,
        Message: response.data.Message,
      });
      ...
      if (response.data.Status === "Success") {
        ...
        return {
          success: false,
          error:
            response.data.Details || response.data.Message || "Invalid OTP",
        };
      }
    } catch (error) {
      ...
      if (error.code === "ECONNABORTED" || error.message.includes("timeout")) {
        errorMessage =
          "OTP verification timed out. Please check your internet connection and try again.";
