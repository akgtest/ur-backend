...
export const verify2FactorAuth = (req, res, next) => {
  try {
    const sessionCookieName =
      process.env.SESSION_COOKIE_NAME || "urbanesta_session";
    ...
    console.info("2Factor authentication successful", { phoneNumber });
    next();
  } catch (error) {
    console.error("2Factor authentication error:", error);
    return res.status(401).json({
      success: false,
